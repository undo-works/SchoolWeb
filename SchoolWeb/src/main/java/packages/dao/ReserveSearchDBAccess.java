...
public class ReserveSearchDBAccess extends ControlDBAccess {
	public ArrayList<T_Order> searchReserve()throws Exception{
		...
		try {
			String sql = 	"SELECT ORDERNO, CUSTID, ORDERDATE, RESERVEDATE, TAXID, STATUS " +
					"FROM T_ORDER WHERE STATUS = 1 AND RESERVEDATE IS NOT NULL "
